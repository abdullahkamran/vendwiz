...
<svelte:head>
	<title>Create account — VendWiz</title>
</svelte:head>
...
<div class="min-h-screen flex items-center justify-center bg-[var(--color-surface-2)] px-4">
	<div class="w-full max-w-[440px]">
		...
		<div class="mb-8 text-center">
			...
			<p class="mt-1 text-sm text-[var(--color-text-muted)]">Create your account</p>
		</div>
		...
		<div class="bg-white rounded-2xl shadow-sm border border-[var(--color-border)] p-8">
			<form onsubmit={handleSubmit} novalidate class="flex flex-col gap-4">
				...
				<div class="flex flex-col gap-1.5">
					<label for="name" class="text-sm font-medium text-[var(--color-text)]">Full name</label>
					<input
						id="name"
						type="text"
						bind:value={name}
				...
				<div class="flex flex-col gap-1.5">
					<label for="password" class="text-sm font-medium text-[var(--color-text)]">
						Password
					</label>
					<input
						id="password"
